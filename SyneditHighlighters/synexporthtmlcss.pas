...
-------------------------------------------------------------------------------}
{
@abstract(Provides a HTML with CSS exporter for SynEdit)
@author(Miguel A. Risco-Castillo)
@created(2025-11-16)
The SynExportHTMLCSS unit provides SynEdit with an exporter that supports HTML and CSS..
Based upon the original SynExportHTML unit.
}


unit SynExportHTMLCSS;
...
uses Dialogs,
  ...
  TExportHtmlCssOptions = set of TExportHtmlCssOption;


  { TSynExporterHTMLCSS }

  TSynExporterHTMLCSS = class(TSynCustomExporter)
  private
    fOptions: TExportHtmlCssOptions;
    fCodeClass: string;
    procedure SeTExportHtmlCssOptions(Value: TExportHtmlCssOptions);
    ...
    procedure SetCodeClass(Value: string);
  protected
...
{end}                                                                           //mh 2000-10-10
  ...
  published
    ...
    property Options: TExportHtmlCssOptions read fOptions write SeTExportHtmlCssOptions default [heoDoctype, heoCharset];
    property Font;
    property Highlighter;
    property Title;
    property UseBackground;
    property CodeClass: string read fCodeClass write SetCodeClass;
  end;
...
constructor TSynExporterHTMLCSS.Create(AOwner: TComponent);
const
...
procedure TSynExporterHTMLCSS.FormatAfterLastAttribute;
begin
...
procedure TSynExporterHTMLCSS.FormatAttributeDone(BackgroundChanged,
...
procedure TSynExporterHTMLCSS.FormatAttributeInit(BackgroundChanged,
  ForegroundChanged: boolean; FontStylesChanged: TFontStyles);
begin
  if BackgroundChanged then
...
{begin}                                                                         //mh 2000-10-10
procedure TSynExporterHTMLCSS.FormatBeforeFirstAttribute(BackgroundChanged,
  ForegroundChanged: boolean; FontStylesChanged: TFontStyles);
begin
  if BackgroundChanged then
    AddData('<span style="background-color: ' +
...
procedure TSynExporterHTMLCSS.FormatBeforeFirstAttributeImmediate(BG, FG: TColor);
var
  Span: String;
begin
  {$ifdef debug_synexporthtml}
  SendDebug('TSynExporterHTMLCSS.FormatBeforeFirstAttributeImmediate');
  {$endif}
...
procedure TSynExporterHTMLCSS.FormatAfterLastAttributeImmediate;
begin
  {$ifdef debug_synexporthtml}
  SendDebug('TSynExporterHTMLCSS.FormatAfterLastAttributeImmediate');
  {$endif}
  if (heoFragmentOnly in Options) then
    AddData('</span>');
end;

procedure TSynExporterHTMLCSS.FormatAttributeInitImmediate(
...
begin
  {$ifdef debug_synexporthtml}
  SendDebug('TSynExporterHTMLCSS.FormatAttributeInitImmediate');
  {$endif}
...
procedure TSynExporterHTMLCSS.FormatAttributeDoneImmediate(
  Attri: TSynHighlighterAttributes; IsSpace: Boolean);
var
  FG, BG: TColor;
  StyleStr: String;
begin
  {$ifdef debug_synexporthtml}
  SendDebug('TSynExporterHTMLCSS.FormatAttributeDoneImmediate');
  {$endif}
...
procedure TSynExporterHTMLCSS.FormatNewLine;
begin
  AddNewLine;
end;

function TSynExporterHTMLCSS.GetFooter: string;
begin
  Result := CodeEnd;
...
function TSynExporterHTMLCSS.GetFormatName: string;
begin
  Result := SYNS_ExporterFormatHTML;
end;

function TSynExporterHTMLCSS.GetHeader: string;
var
  DocHeader, CodeStartClass, HeadText, WinClipHeader, SFooter: String;
  WinClipHeaderSize, FooterLen: Integer;
begin
  Result := '';
  DocHeader:= '';
  CodeStartClass := Format(CodeStart,[CodeClass]);
  if not (heoFragmentOnly in Options) then
  begin
    ...
    DocHeader := DocHeader + CodeStartClass; //Don't add LineEndings after this point, because of <pre> tag
  end  //not heoFragmentOnly
  else
  begin
    if (heoWinClipHeader in fOptions) then
      DocHeader := DocHeader + StartFragmentComment + CodeStartClass
    else
      DocHeader := DocHeader + CodeStartClass;
  end;
  ...
  begin
    ...
    {$ifdef debug_synexporthtml}
    SendDebug('TSynExporterHTMLCSS.GetHeader: WinClipHeaderSize=' + IntToStr(WinClipHeadersize));
    SendDebug('  Footer="' + SFooter + '"');
    SendDebug('  FooterLen=' + IntToStr(FooterLen));
    SendDebug('  BufferSize=' + IntToStr(getBufferSize));
    SendDebug('  length(docHeader)=' + IntToStr(length(docheader)));
    {$endif}
...
procedure TSynExporterHTMLCSS.SetExportAsText(Value: boolean);
begin
...
procedure TSynExporterHTMLCSS.SeTExportHtmlCssOptions(Value: TExportHtmlCssOptions);
begin
...
function TSynExporterHTMLCSS.GetCreateHTMLFragment: Boolean;
begin
  Result := (heoFragmentOnly in fOptions);
end;

procedure TSynExporterHTMLCSS.SetCreateHTMLFragment(Value: Boolean);
begin
...
function TSynExporterHTMLCSS.MakeFontSpan(FG, BG: TColor): String;
begin
...
function TSynExporterHTMLCSS.StyleToHtml(AStyle: TFontStyles; IsSpace, DoSet: Boolean): String;
begin
  Result := '';
  if DoSet then
  begin
...
procedure TSynExporterHTMLCSS.SetCodeClass(Value: string);
begin
  fCodeClass:= Value;
end;
