...
begin
  ...
  if HashKey < 207 then Result := fIdentFuncTable[HashKey]{$ifdef FPC} () {$endif} else
    Result := tkIdentifier;
end;
...
{$IFDEF SYN_LAZARUS}
procedure TSynSTSyn.GetTokenEx(out TokenStart: PChar; out TokenLength: integer);
begin
...
procedure TSynSTSyn.SetLine(const NewValue: string; LineNumber:Integer);
begin
