...
procedure TSynCSSyn.SetLine(const NewValue: String; LineNumber:Integer);
begin
...
{$IFDEF SYN_LAZARUS}
procedure TSynCSSyn.GetTokenEx(out TokenStart :PChar; out TokenLength :integer);
begin
...
begin
  Result := '// Hello3.cs'+LineEnding+
            '// arguments: A B C D'+LineEnding+
            'using System;'+LineEnding+
            ''+LineEnding+
            'public class Hello3'+LineEnding+
            '{'+LineEnding+
            '   public static void Main(string[] args)'+LineEnding+
            '   {'+LineEnding+
            '      Console.WriteLine("Hello, World!");'+LineEnding+
            '      Console.WriteLine("You entered the following {0} command line arguments:",'+LineEnding+
            '         args.Length );'+LineEnding+
            '      for (int i=0; i < args.Length; i++)'+LineEnding+
            '      {'+LineEnding+
            '         Console.WriteLine("{0}", args[i]);'+LineEnding+
            '      }'+LineEnding+
            '   }'+LineEnding+
            '}'+LineEnding+
            ''+LineEnding+
            '//Catching an exception'+LineEnding+
            'int top = 0, bottom = 0, result = 0;'+LineEnding+
            'try'+LineEnding+
            '{'+LineEnding+
            '    result = top / bottom;'+LineEnding+
            '}'+LineEnding+
            'catch (System.DivideByZeroException ex)'+LineEnding+
            '{'+LineEnding+
            '    Console.WriteLine("{0} Exception caught.", ex);'+LineEnding+
            '}'+LineEnding+
            ''+LineEnding+
            '//DECLARE A PROPERTY'+LineEnding+
            'private string name;'+LineEnding+
            '// A read-write instance property:'+LineEnding+
            'public string NameProperty'+LineEnding+
            '{'+LineEnding+
            '   get'+LineEnding+
            '   {'+LineEnding+
            '      return name;'+LineEnding+
            '   }'+LineEnding+
            '   set'+LineEnding+
            '   {'+LineEnding+
            '      name = value;'+LineEnding+
            '   }'+LineEnding+
            '}'+LineEnding
