...
begin
  ...
  fIdentFuncTable[1] :=  {$IFDEF FPC}@{$ENDIF}  Func1;
  fIdentFuncTable[2] :=  {$IFDEF FPC}@{$ENDIF}  Func2;
...
begin
  for I := #0 to #255 do
    case I of
      ...
      'A'..'Z', 'a'..'z', '%', '^': fProcTable[I] := {$IFDEF FPC}@{$ENDIF} IdentProc;
      '$': fProcTable[i] :=  {$IFDEF FPC}@{$ENDIF}  FuncProc;
      '@': fProcTable[i] := {$IFDEF FPC}@{$ENDIF} IndirectProc;
      ...
      '(',')','+','-','[',']','.','<','>','''','=',',',':','/','\',
      '?','!','_','*': fProcTable[i] := {$IFDEF FPC}@{$ENDIF} SymbolProc;
      '#': fProcTable[i] := {$IFDEF FPC}@{$ENDIF} DirectiveProc;
      '&': fProcTable[i] := {$IFDEF FPC}@{$ENDIF} EmbeddedProc;
...
{$IFDEF SYN_LAZARUS}
procedure TSynCacheSyn.GetTokenEx(out TokenStart: PChar; out TokenLength: integer);
begin
...
procedure TSynCacheSyn.SetLine(const NewValue: string; LineNumber: Integer);
begin
