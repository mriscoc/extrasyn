...
begin
  ...
  fIdentFuncTable[238] :=  {$IFDEF FPC}@{$ENDIF}  Func238;
end;
...
begin
  ...
  if HashKey < MAXIDENTTABLE + 1 then Result := fIdentFuncTable[HashKey]{$IFDEF FPC}(){$ENDIF} else Result := tkIdentifier;
end;
...
begin
  for I := #0 to #255 do
    case I of
      ...
      '$', '@': fProcTable[I] :=  {$IFDEF FPC}@{$ENDIF}  DollarSignProc;
...
{$IFDEF SYN_LAZARUS}
procedure TSynUnrealSyn.GetTokenEx(out TokenStart: PChar; out TokenLength: integer);
begin
...
procedure TSynUnrealSyn.SetLine(const NewValue: String; LineNumber:Integer);
begin
